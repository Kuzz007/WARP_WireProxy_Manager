...
update_local_scripts() { need_root; need_curl; log "Обновляю локальный native-скрипт..."; safe_download_exec "$NATIVE_URL" "$NATIVE_BIN"; ok "Обновлён: $NATIVE_BIN"; log "Обновляю менеджер атомарно..."; safe_download_exec "$MANAGER_URL" "$MANAGER_BIN"; ok "Обновлён: $MANAGER_BIN"; }
...
scheduler_status() { local cron timer_active timer_enabled timer_minutes scheduler; cron="$(cron_installed_bool)"; timer_active="$(timer_active_bool)"; timer_enabled="$(timer_enabled_bool)"; timer_minutes="$(get_timer_minutes)"; if [[ "$cron" == "1" && "$timer_active" == "1" ]]; then scheduler="both"; elif [[ "$cron" == "1" ]]; then scheduler="cron"; elif [[ "$timer_active" == "1" ]]; then scheduler="systemd_timer"; else scheduler="none"; fi; echo "============================================================"; echo "SCHEDULER STATUS"; echo "============================================================"; echo "scheduler: $scheduler"; echo "cron installed: $cron"; echo "timer active: $timer_active"; echo "timer enabled: $timer_enabled"; echo "timer interval minutes: $timer_minutes"; [[ -f "$CRON_FILE" ]] && { echo; echo "--- cron ---"; cat "$CRON_FILE"; }; [[ -f "$TIMER_FILE" ]] && { echo; echo "--- timer ---"; cat "$TIMER_FILE"; }; }
...
wg_conf_to_json() {
  local file="${1:-}" line section key value private_key address mtu public_key endpoint allowed_ips keepalive workers no_kernel_tun
  if [[ -z "$file" && -t 0 ]]; then read -rp "Путь к WireGuard .conf: " file; fi
  if [[ -z "$file" || ! -f "$file" ]]; then err "Файл WireGuard .conf не найден: ${file:-empty}"; echo "Пример: warpwp --wg-json /root/wg0.conf"; return 1; fi
  mtu="1420"; keepalive="0"; workers="2"; no_kernel_tun="false"; section=""
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$section:$key" in
      ...
      interface:address) address="$value" ;;
  ...
  if [[ -z "${private_key:-}" || -z "${address:-}" || -z "${public_key:-}" || -z "${endpoint:-}" ]]; then
    err "Не хватает обязательных полей: PrivateKey, Address, Peer PublicKey, Endpoint."
    return 1
  fi
  [[ -n "${allowed_ips:-}" ]] || allowed_ips="0.0.0.0/0, ::/0"
  echo "============================================================"
  echo "3x-ui / Xray WireGuard JSON"
  echo "============================================================"
  echo "Вставь этот блок в окно создания outbound на вкладке JSON."
  echo
  printf '{\n'
  ...
  printf '    "address": [\n'
  IFS=',' read -ra addr_arr <<< "$address"
  local i item total
  total="${#addr_arr[@]}"
  for i in "${!addr_arr[@]}"; do item="$(trim "${addr_arr[$i]}")"; printf '      "%s"' "$(json_escape "$item")"; [[ "$i" -lt $((total-1)) ]] && printf ','; printf '\n'; done
  printf '    ],\n'
  printf '    "workers": %s,\n' "$workers"
  printf '    "peers": [\n'
  printf '      {\n'
  ...
  total="${#allowed_arr[@]}"
  for i in "${!allowed_arr[@]}"; do item="$(trim "${allowed_arr[$i]}")"; printf '          "%s"' "$(json_escape "$item")"; [[ "$i" -lt $((total-1)) ]] && printf ','; printf '\n'; done
  printf '        ],\n'
  ...
  printf '}\n'
  echo
  echo "Подсказка: после создания outbound задай tag, например WG, и используй routing на outboundTag WG."
}
...
============================================================
  ...
  warpwp --wg-json FILE     # конвертировать WireGuard .conf в JSON для 3x-ui
  warpwp --logs             # логи
...
WG JSON: warpwp --wg-json /path/to/wg.conf
