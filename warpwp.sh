...
get_timer_minutes() {
  ...
  if [[ -f "$TIMER_ENV_FILE" ]]; then
    value="$(grep -E '^TIMER_MINUTES=' "$TIMER_ENV_FILE" 2>/dev/null | tail -n1 | cut -d= -f2- | tr -d '"' || true)"
  fi
  [[ "$value" =~ ^[0-9]+$ ]] || value="$DEFAULT_TIMER_MINUTES"
  echo "$value"
}
...
ask_timer_minutes() {
  ...
  if [[ -t 0 ]]; then
    read -rp "Интервал проверки в минутах [${current}]: " input || true
    input="${input:-$current}"
  else
    input="${1:-$current}"
  fi
  if ! [[ "$input" =~ ^[0-9]+$ ]] || [[ "$input" -lt 1 ]]; then
    warn "Некорректный интервал '$input', использую ${DEFAULT_TIMER_MINUTES} минут."
    input="$DEFAULT_TIMER_MINUTES"
  fi
  echo "$input"
}
...
update_local_scripts() {
  need_root; need_curl
  log "Обновляю локальный native-скрипт..."; safe_download_exec "$NATIVE_URL" "$NATIVE_BIN"; ok "Обновлён: $NATIVE_BIN"
  log "Обновляю менеджер атомарно..."; safe_download_exec "$MANAGER_URL" "$MANAGER_BIN"; ok "Обновлён: $MANAGER_BIN"
}

install_cron_check() {
  need_root; ensure_flock
  [[ -x "$NATIVE_BIN" ]] || { warn "Локальный native-скрипт не найден. Сначала обновляю скрипты."; update_local_scripts; }
  log "Переключаю scheduler на cron: systemd timer будет отключён."
  remove_timer_check_quiet
  local check_cmd
...
EOF_CRON
  ...
  ok "Systemd timer отключён, чтобы не было двойного scheduler."
}

install_timer_check() {
  need_root; ensure_flock
  [[ -x "$NATIVE_BIN" ]] || { warn "Локальный native-скрипт не найден. Сначала обновляю скрипты."; update_local_scripts; }
  local minutes exec_cmd
  minutes="$(ask_timer_minutes "${1:-}")"
  log "Переключаю scheduler на systemd timer: cron будет отключён."
  remove_cron_check
  mkdir -p "$(dirname "$TIMER_ENV_FILE")"
  cat > "$TIMER_ENV_FILE" <<EOF_ENV
...
EOF_TIMER
  chmod 0644 "$TIMER_SERVICE_FILE" "$TIMER_FILE" "$TIMER_ENV_FILE"
  systemctl daemon-reload
  systemctl enable --now warp-wireproxy-check.timer
  ok "Timer включён: warp-wireproxy-check.timer, интервал: ${minutes} минут"
  ok "Cron отключён, чтобы не было двойного scheduler."
}

remove_timer_check() { need_root; log "Отключаю systemd timer..."; remove_timer_check_quiet; ok "Systemd timer удалён. Cron не тронут."; }

scheduler_status() {
  local cron timer_active timer_enabled timer_minutes scheduler
  cron="$(cron_installed_bool)"; timer_active="$(timer_active_bool)"; timer_enabled="$(timer_enabled_bool)"; timer_minutes="$(get_timer_minutes)"
  if [[ "$cron" == "1" && "$timer_active" == "1" ]]; then scheduler="both"
  elif [[ "$cron" == "1" ]]; then scheduler="cron"
  elif [[ "$timer_active" == "1" ]]; then scheduler="systemd_timer"
  ...
  echo "timer interval minutes: $timer_minutes"
  [[ -f "$CRON_FILE" ]] && { echo; echo "--- cron ---"; cat "$CRON_FILE"; }
  [[ -f "$TIMER_FILE" ]] && { echo; echo "--- timer ---"; cat "$TIMER_FILE"; }
}
...
status() {
  echo "============================================================"; echo "СОСТОЯНИЕ WARP / WIREPROXY"; echo "============================================================"
  echo "warpwp v$VERSION"; [[ -x "$NATIVE_BIN" ]] && "$NATIVE_BIN" --version 2>/dev/null || echo "native script: not installed"
  echo; echo "--- Endpoint ---"; grep -i '^Endpoint' /etc/wireguard/warp.conf 2>/dev/null || echo "warp.conf не найден"
  ...
  echo; scheduler_status; echo; print_memo_short
}

status_json() {
  local ep ep_port native_ver service_state service_active socks_listening cron_installed cron_flock log_exists manager_installed native_installed trace ip colo loc warp installed healthy timer_installed timer_active timer_enabled timer_minutes timer_log_exists scheduler
  ep="$(current_endpoint)"; ep_port="$(current_endpoint_port)"; native_ver="$(native_version)"; service_state="$(systemctl is-active wireproxy 2>/dev/null || true)"
  ...
  cron_installed="$(cron_installed_bool)"; cron_flock="$(cron_flock_bool)"; [[ -f "$LOG_FILE" ]] && log_exists="1" || log_exists="0"
  [[ -x "$MANAGER_BIN" ]] && manager_installed="1" || manager_installed="0"; [[ -x "$NATIVE_BIN" ]] && native_installed="1" || native_installed="0"; [[ -f /etc/wireguard/warp.conf && -f /etc/wireguard/proxy.conf ]] && installed="1" || installed="0"
  timer_installed="$(timer_installed_bool)"; timer_active="$(timer_active_bool)"; timer_enabled="$(timer_enabled_bool)"; timer_minutes="$(get_timer_minutes)"; [[ -f "$TIMER_LOG_FILE" ]] && timer_log_exists="1" || timer_log_exists="0"
  if [[ "$cron_installed" == "1" && "$timer_active" == "1" ]]; then scheduler="both"; elif [[ "$cron_installed" == "1" ]]; then scheduler="cron"; elif [[ "$timer_active" == "1" ]]; then scheduler="systemd_timer"; else scheduler="none"; fi
  trace="$(curl -m 10 -s -x "socks5h://$SOCKS_HOST:$SOCKS_PORT" https://www.cloudflare.com/cdn-cgi/trace 2>/dev/null || true)"
  ip="$(echo "$trace" | awk -F= '$1=="ip"{print $2; exit}')"; colo="$(echo "$trace" | awk -F= '$1=="colo"{print $2; exit}')"; loc="$(echo "$trace" | awk -F= '$1=="loc"{print $2; exit}')"; warp="$(echo "$trace" | awk -F= '$1=="warp"{print $2; exit}')"
  [[ "$installed" == "1" && "$service_active" == "1" && "$socks_listening" == "1" && "$warp" == "on" && "$scheduler" != "none" ]] && healthy="1" || healthy="0"
  cat <<EOF_JSON
{
...
doctor() {
  ...
  local ok_count=0 warn_count=0 fail_count=0 scheduler trace
  ...
  command -v flock >/dev/null 2>&1 && check_ok "flock найден" || check_warn "flock не найден"
  ...
  if [[ "$(cron_installed_bool)" == "1" && "$(timer_active_bool)" == "1" ]]; then scheduler="both"; check_warn "активны оба scheduler: cron и timer. Исправить: warpwp --install-cron или warpwp --install-timer"
  elif [[ "$(cron_installed_bool)" == "1" ]]; then scheduler="cron"; check_ok "scheduler: cron"
  elif [[ "$(timer_active_bool)" == "1" ]]; then scheduler="systemd_timer"; check_ok "scheduler: systemd timer"
  else scheduler="none"; check_fail "scheduler не установлен"; fi
  [[ "$scheduler" == "cron" && "$(cron_flock_bool)" == "1" ]] && check_ok "cron использует flock lock" || true
  [[ "$scheduler" == "systemd_timer" ]] && check_ok "timer interval: $(get_timer_minutes) min" || true
  [[ -f "$LOG_FILE" || -f "$TIMER_LOG_FILE" ]] && check_ok "лог существует" || check_warn "логи пока отсутствуют"
...
Cron mode: warpwp --install-cron
Timer mode: warpwp --install-timer
