...
# Автоустановка/настройка Cloudflare WARP через wireproxy + подбор fastest endpoint + вывод блоков для 3x-ui/Xray и zapret4rocket.
#
# Проверено под Debian/Ubuntu systemd VPS.
# Использует fscarmen/warp menu.sh option "w" для установки WireProxy, если wireproxy ещё не установлен.
#
# Запуск:
#   bash warp-wireproxy-auto.sh
#
# Опции:
#   bash warp-wireproxy-auto.sh --port 40000
#   bash warp-wireproxy-auto.sh --no-install
#   bash warp-wireproxy-auto.sh --endpoints "162.159.192.244:1843 162.159.195.100:1010 162.159.193.10:2408"
...
detect_os_and_install_deps() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update -y
    DEBIAN_FRONTEND=noninteractive apt-get install -y curl wget ca-certificates grep sed awk coreutils iproute2 systemd
    ...
    warn "Unknown package manager. Make sure curl, wget, sed, awk, iproute2 and systemd are installed."
  fi
}

wireproxy_exists() {
  command -v wireproxy >/dev/null 2>&1 || [[ -x /usr/bin/wireproxy ]] || systemctl list-unit-files 2>/dev/null | grep -q '^wireproxy\.service'
}
...
install_wireproxy_if_needed() {
  ...
  log "wireproxy or configs are missing. Running fscarmen WARP WireProxy installer..."
  warn "This installer is external. It may ask questions depending on the OS/environment."
  cd /root
  wget -N "$FSCARMEN_MENU_URL" -O /root/menu.sh
  bash /root/menu.sh w || {
    err "fscarmen installer failed. Try manually:"
    err "wget -N $FSCARMEN_MENU_URL && bash menu.sh w"
    exit 1
  }
...
test_endpoint() {
  ...
  local time_file="/tmp/warp_time.$$"
  local rc=0

  set_endpoint "$ep"
  restart_wireproxy

  # First request: capture trace and timing.
